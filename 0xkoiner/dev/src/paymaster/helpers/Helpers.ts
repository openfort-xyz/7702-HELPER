...
import { SIMPLE_ACCOUNT, USDC_BASE_SEPOLIA, PAYMASTER_V3, TREASURY } from "../data/addressBook";
...
class Helpres {
  ...
  async getFreshUserOp(authorization: SignAuthorizationReturnType): Promise<UserOperationWithEip7702Auth> {
    return {
      sender: '0x',
      nonce: 0n,
      // factory field removed - permissionless library handles EIP-7702 factory logic internally
      callData: '0x',
      callGasLimit: 0n,
  ...
  async getCalBatchData(): Promise<EncodeFunctionDataReturnType> {
    const approveData = encodeFunctionData({
      abi: this.ERC20_APPROVE_ABI,
      functionName: "approve",
      args: [PAYMASTER_V3, this.MAX_UINT256]
    });
  ...
  getDummyPaymasterDataERC20(
    ...
    const paymasterData = this.createVerifyingModePaymasterDataERC20({
      ...
      exchangeRate,
      paymasterValidationGasLimit,
      treasury: TREASURY, // Address where USDC tokens will be sent
  ...
  createAsyncPaymasterDataWithMagic(validUntil: number, validAfter: number): Hex {
    const modeByte = '0x00';
    const validUntilHex = pad(toHex(validUntil), { size: 6 });
    const validAfterHex = pad(toHex(validAfter), { size: 6 });
  ...
  createAsyncPaymasterDataForSigning(validUntil: number, validAfter: number): Hex {
    const modeByte = '0x00';
    const validUntilHex = pad(toHex(validUntil), { size: 6 });
    const validAfterHex = pad(toHex(validAfter), { size: 6 });
    const sigLengthPlaceholder = toHex(0, { size: 2 });
    return concat([modeByte, validUntilHex, validAfterHex, sigLengthPlaceholder, this.PAYMASTER_SIG_MAGIC]) as Hex;
  }

  appendAsyncSignature(paymasterDataBase: Hex, signature: Hex): Hex {
    const signatureBytes = signature.slice(2);
    const sigLength = toHex(signatureBytes.length / 2, { size: 2 });
    return concat([paymasterDataBase, signature, sigLength, this.PAYMASTER_SIG_MAGIC]) as Hex;
  }

  getDummyPaymasterDataAsync(validUntil: number, validAfter: number): Hex {
    const modeByte = '0x00';
    const validUntilHex = pad(toHex(validUntil), { size: 6 });
    const validAfterHex = pad(toHex(validAfter), { size: 6 });
    const sigLength = toHex(65, { size: 2 });
    return concat([modeByte, validUntilHex, validAfterHex, this.DUMMY_SIG, sigLength, this.PAYMASTER_SIG_MAGIC]) as Hex;
  }
}
